...
class QuestSystem:
    ...
    def get_active_quest_descriptions(self, flags: Optional[Set[str]] = None) -> List[str]:
        ...
        if named and owed:
            ...
            return [f"Имя и долг{whom} сказаны. Туман на востоке улицы уже слышал. Слышал — и молчит."]
        if named:
            return ["Имя есть. Долг ещё в коридоре. Туман на востоке ждёт оба. Ждёт — и не торопится."]
        if owed:
            whom = face or "долг"
            return [f"Долг — {whom}. Имени туман не слышал. Не слышал, и в этом вся стена."]
        return ["Вспомнить имя. И долг. Без обоих улица — стена. Стена сытая."]
